...
function runIt() {
    ...
    var pictureMetaExisting = [];
    var metaFileStr = null;
    try {
        metaFileStr = fsPkg.readFileSync(yargsCmds.pictureMetaFile, 'utf8');
    }
    catch (err) {
        console.log('file \"' + yargsCmds.pictureMetaFile + '\" not present. Building a new file');
    }
    // metadata may not exist, so test if data is there...
    if (metaFileStr)
        pictureMetaExisting = JSON.parse(metaFileStr).lookup;
    ...
    const blackYellow = '\x1b[30;103;1m';
    const RESET = '\x1b[0m'; // ansi escape codes
    try {
        fsPkg.writeFileSync(yargsCmds.menuOut, menuStr);
    }
    catch (err) {
        console.log(blackYellow + 'ERROR write file: ' + err.message);
        return;
    }
 ...
 */
function readAJSON_recurs(dir, pictureMetaExisting) {
...
function consecutiveBackup(fileToBackupPath, backupDir) {
    var _a;
    // typescript optional chaining ?. "QUESTIONMARK AND DOT !!!!!!"
    var fileToBackupNoPath = (_a = fileToBackupPath.match(/.*\/(.*)/)) === null || _a === void 0 ? void 0 : _a[1];
    var bakNum = GetNextBackupNumber(backupDir, fileToBackupNoPath);
    ...
    try {
        fsPkg.copyFileSync(fileToBackupPath, fileToBackupDestinationPath, fsPkg.constants.COPYFILE_EXCL);
    }
    catch (ex) {
        if (ex.code == 'ENOENT')
...
function GetNextBackupNumber(backupDir, fileNameBefore_bk3) {
    const files = fsPkg.readdirSync(backupDir);
    const numbers = files.map((fileN) => {
        ...
        if (!match)
            return 0; // this file is a backup of some other file
 ...
 */ 
//# sourceMappingURL=treeToMenu.js.map