...
if (!yargsCmds.PIXFULL || !yargsCmds.PIXMINI) {
    console.log('PIXFULL or PIXMINI not specified. Quitting. ');
    process.exit();
}
if (yargsCmds.DRY_RUN != 'true') { // set this way when developing ie VSCode
    ...
    var mat = readlineSync.question().match(/^(y|Y)$/); // ^ is start, match y or Y then end of string
...
function copyFoldersWithMods(srcRootPath, destinationRootDir, LIVE_RUN) {
 ...
 */
function recurseFolders(srcDirRecurseLevel, srcRootPath, destinationRootDir, LIVE_RUN) {
    ...
    files.forEach((fileOrDir) => {
        ...
        if (fileOrDir.isDirectory()) {
            ...
            if (LIVE_RUN) {
                if (!fsPkg.existsSync(destPath_fullSize))
                    fsPkg.mkdirSync(destPath_fullSize, { recursive: false });
                if (!fsPkg.existsSync(destPath_miniSize))
                    fsPkg.mkdirSync(destPath_miniSize, { recursive: false });
            }
...
function showFullCommandLine() {
    ...
    console.log('command line: ' + BRIGHTYELLOW + outstr);
}
/* trash
/////////////////////////////////
