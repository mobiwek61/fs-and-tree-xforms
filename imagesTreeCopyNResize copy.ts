...
const RESIZE_PX=887
// in VSCode, launch.json must have this ->      "console": "integratedTerminal",
var DRY_RUN=true;
const DEST_ROOT = 'xformedImgs'
...
// } 
var outstr=''; process.argv.forEach((val, index) => {
...
function runIt() {  // read in existing picture metadata (ie: caption) and add if necessary if file was added or name change
  ...
  //  return
  //}
  if (!fsPkg.existsSync('outputFiles')) {
      console.log('folder /"outputfiles/" must exist. To enforce only down-tree writes for safety reasons.')
      process.exit()
  }
  if (!DRY_RUN) { 
    fsPkg.mkdirSync('outputFiles/newImageTree/fullSize/jpeg', { recursive: true });
    fsPkg.mkdirSync('outputFiles/newImageTree/miniSize/jpeg', { recursive: true });
  }
...
function recurseFolders(srcDirRecurseLevel: string, srcRootPath: string, destinationRootDir: string, LIVE_RUN: boolean) { 
  ...
  files.forEach((fileOrDir:any) => {
      ...
      var sourcePath = srcDirRecurseLevel.replace(srcRootPath, DEST_ROOT) // remove original root
      // var sourcePath = srcDirRecurseLevel.replace(/\.*?\/{srcRootPath}\//, '') // remove original root
      
      sourcePath =  sourcePath + '/' + fileOrDir.name // path.join(dir, file.name);
      // remove source root path because we dont want it in destination path
      
      const destPath_fullSize = destinationRootDir + 'fullSize/' + sourcePath
      //   
      ...
      if (fileOrDir.isDirectory()) {
        ...
        if (LIVE_RUN) {
          if (!sourcePath.match(/.*(\.jpg|\.JPG|\.jpeg|\.JPEG)/)) {
              ...
              ResizeImage(sourcePath, RESIZE_PX, destPath_fullSize)
              console.log('resized')
          }
        }
      };
